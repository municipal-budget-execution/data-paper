...
fetch_licitacoes_window <- function(date_start, date_end,
  ...
  base_url <- "https://dadosabertos.compras.gov.br/modulo-legado/3_consultarPregoes"
  page <- 1
  out <- list()
  ...
  repeat {
    q <- c(
      list(
        ...
        dt_data_edital_inicial = format(as.Date(date_start), "%Y-%m-%d"),
        dt_data_edital_final   = format(as.Date(date_end),   "%Y-%m-%d")
      ),
      extra_params
    )
...
pregoes_2024 <- get_licitacoes("2021-01-01", "2021-12-31")
