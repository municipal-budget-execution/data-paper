...
data[ano!= 2020 & ano != 2014, .(total_compras_licitacao = sum(total_compras_licitacao),
         total_compras_empenho = sum(total_compras_empenho)), 
     by = id_municipio] %>% 
  mutate(ratio_lic  = 100*(total_compras_licitacao/total_compras_empenho-1)) %>% 
  mutate(ratio_lic = pmin(ratio_lic, 300)) %>% 
  ggplot() + 
  geom_histogram(aes(x = ratio_lic,
                     y = stat(width*density)), binwidth = 5, 
                 color = "#0D3446",
